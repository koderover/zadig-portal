        ...
        <div class="highlighter-rouge">
          <div class="highlight">
            <span class="code-line">
              {{`$ kubectl apply -f "${$utils.getOrigin()}/api/aslan/cluster/agent/${accessCluster.id}/agent.yaml?kubeVersion=$(kubectl version --short | grep -i Server | awk '{print $NF}' | tr -d '\n')${deployType==='Deployment'?'&type=deploy':''}"`}}
...
export default {
  ...
  methods: {
    ...
    getYamlUrl () {
      return `kubectl apply -f "${this.$utils.getOrigin()}/api/aslan/cluster/agent/${this.accessCluster.id}/agent.yaml?kubeVersion=$(kubectl version --short | grep -i Server | awk '{print $NF}' | tr -d '\n')${this.deployType === 'Deployment' ? '&type=deploy' : ''}"`
    },
